...
TcpStream::TcpStream(const std::string& host, int port)
{
    ...
    if (connect(sock, servinfo->ai_addr, static_cast<int>(servinfo->ai_addrlen)) == -1) {
...
public:
    ...
    {
        while (bufferSize > 0) {
            RECV_SEND_T sent = send(tcpStream->sock, buffer + bufferPos, static_cast<int>(bufferSize), 0);
            if (sent < 0) {
